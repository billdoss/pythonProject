...
def vp_start_gui():
    ...
    global val, w, root
    root = Tk()
    Form_prod_support.set_Tk_var()
    top = Mise_à_jour_forme_produit (root)
    Form_prod_support.init(root, top)
    root.mainloop()

w = None
def create_Mise_à_jour_forme_produit(root, *args, **kwargs):
    '''Starting point when module is imported by another program.'''
    global w, w_win, rt
    rt = root
    w = Toplevel (root)
    Form_prod_support.set_Tk_var()
    top = Mise_à_jour_forme_produit (w)
    Form_prod_support.init(w, top, *args, **kwargs)
    return (w, top)

def destroy_Mise_à_jour_forme_produit():
    global w
    w.destroy()
    w = None


class Mise_à_jour_forme_produit:
    def maj():
        Form_prod_support.heure.set(time.strftime('%H:%M:%S'))
        root.after(1000,Mise_à_jour_forme_produit.maj)
    def __init__(self, top=None):

        def inserer():

            self.listead.insert('', 1, values=(Form_prod_support.code.get(),Form_prod_support.lib.get()))
        ...
        top.title("Mise à jour forme produit")
        ...
        self.Labelframe1.configure(text='''Info/Forme produit''')
        ...
        self.Entry1.configure(textvariable=Form_prod_support.code)
        ...
        self.Entry2.configure(textvariable=Form_prod_support.lib)
        ...
        self.Button1.bind('<Button-1>',lambda e:Form_prod_support.Valider(e))
        ...
        self.Labelframe4.configure(text='''Liste des formes de produit''')
        ...
        self.Label8.configure(textvariable=Form_prod_support.heure)
        Mise_à_jour_forme_produit.maj()
