...
def vp_start_gui():
    ...
    global val, w, root
    root = Tk()
    Grades_support.set_Tk_var()
    top = Mise_à_jour_grade (root)
    Grades_support.init(root, top)
    root.mainloop()

w = None
def create_Mise_à_jour_grade(root, *args, **kwargs):
    '''Starting point when module is imported by another program.'''
    global w, w_win, rt
    rt = root
    w = Toplevel (root)
    Grades_support.set_Tk_var()
    top = Mise_à_jour_grade (w)
    Grades_support.init(w, top, *args, **kwargs)
    return (w, top)

def destroy_Mise_à_jour_grade():
    global w
    w.destroy()
    w = None


class Mise_à_jour_grade:
    def maj():
        Grades_support.heure.set(time.strftime('%H:%M:%S'))
        root.after(1000,Mise_à_jour_grade.maj)
    def __init__(self, top=None):
        ...
        self.Labelframe1.configure(text='''Info/grade''')
        ...
        self.Button1.bind('<Button-1>',lambda e:Grades_support.Valider(e))
        ...
        self.Labelframe4.configure(text='''Liste des grades''')
        ...
        self.Label8.configure(textvariable=Grades_support.heure)
        Mise_à_jour_grade.maj()
